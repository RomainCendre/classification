...
def vae():
    ...
    z = Lambda(sampling, output_shape=(latent_dim,), name='features_output')([z_mean, z_log_var])
    ...
    outputs = Reshape(original_dim, name='reconstruction_output')(outputs)
    ...
    def features_loss(x_feat, y_true):
        return davies_bouldin_score(x_feat, y_true)

    losses = {
        'reconstruction_output': vae_loss,
        'features_output': features_loss,
    }
    lossWeights = {'reconstruction_output': 1.0, 'features_output': 1.0}
    vae.compile(optimizer='adam', loss=losses, loss_weights=lossWeights)
...
x_test_encoded = encoder.predict_generator(validation_generator)
